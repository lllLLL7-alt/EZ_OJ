...
public class JudgeController {
     ...
     */
    @PostMapping("/submit")
    public SubmitTaskResponse submit(@RequestBody JudgeRequest request, HttpSession session) throws Exception {
        // 1. 鉴权：先判断是否登录
        ...
        // 【核心修改】给拓展后的JudgeRequest设置用户ID（从Session取，安全，防止前端伪造）
        request.setUserId(currentUser.getId());

        // 2. 提交任务（直接传request即可，userId已经在里面了）
        String taskId = judgeScheduler.submitJudgeTaskToHolder(request);
        log.info("用户{}提交判题任务成功，任务ID：{}", currentUser.getUsername(), taskId);
